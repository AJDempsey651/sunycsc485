from sunycsc485.projects.hw11.hw11p2 import get_formal_name
import pytest
...
def test_multiple_arguments():
    """Test that if more than one positional arguements are given it
    will result in
    a TypeError"""
    with pytest.raises(Exception) as e:
        get_formal_name(('apple') == ('Malus domestica'),
                        ('banana') == ('Musa acuminata'))
    assert e.type == TypeError


def test_key_error():
    """Test that a positional argument(key) being given that is not in the
     dictionary will return a KeyError"""
    with pytest.raises(Exception) as e:
...
def test_key_does_not_equal():
    """Test that a positional argument(key) will only return it's assigned
    value and
     not another value in the dictionary"""
